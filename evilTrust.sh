...
function ctrl_c(){
	...
	iwconfig wlan0 mode monitor 2>/dev/null; sleep 1
	ifconfig wlan0 up 2>/dev/null; airmon-ng stop wlan0 > /dev/null 2>&1; sleep 1
...
function guiMode(){
	...
	tput civis; airmon-ng start $choosed_interface > /dev/null 2>&1; choosed_interface="${choosed_interface}"
